# Vignesh Yemul Portfolio

A dark, motion-forward personal portfolio for Vignesh Yemul, built with React, TypeScript, Vite, Tailwind CSS, Framer Motion, and Lucide icons.
...
**Resume:** Place your PDF at:

```text
public/Vignesh_Yemul_Resume.pdf
```

The hero "Download Resume" button reads this path from `profile.resumePath` in `portfolio.json`.

**Profile photo:** Replace the placeholder SVG or add your own image:

```text
src/assets/img/vignesh_placeholder.svg
```

Then update the import in `src/components/HeroSection.tsx` to point to your image file (e.g. `vignesh_profile.jpg`).
